...
#include "cthulhu_postboot_plugin.h"
...
Result CthulhuPostBoot_LoadPlugin(Handle process, u32 pid, u32 *threadIdOut)
{
    ...
    if (!process || !pid || !threadIdOut)
        return MAKERESULT(RL_PERMANENT, RS_INVALIDARG, RM_LDR,
                          RD_INVALID_ADDRESS);
    if (PLG__GetConfigMemoryStatus() != PLG_CFG_NONE || PluginLoaderCtx.target)
        return MAKERESULT(RL_PERMANENT, RS_INVALIDSTATE, RM_LDR,
    ...
    if (!TryToLoadPlugin(process, false))
    {
        ...
        if (PluginLoaderCtx.error.message)
            g_lastStage = PluginLoaderCtx.error.message;
        PluginLoaderCtx.target = 0;
        PluginLoaderCtx.pluginIsHome = false;
        return R_FAILED(loadResult) ? loadResult :
            MAKERESULT(RL_PERMANENT, RS_NOTFOUND, RM_LDR, RD_NOT_FOUND);
    }
    // HOME Menu is a persistent system application. Never make shutdown wait
    // for CTRPF event replies if this experimental bootstrap fails partway.
    ...
    for (;;)
    {
        ...
        if (info.type == DBGEVENT_ATTACH_THREAD && selectedThread == 0)
        {
            ...
            if (R_SUCCEEDED(svcGetDebugThreadContext(
                    &context, debug, info.thread_id, THREADCONTEXT_CONTROL_ALL)) &&
    ...
    if (R_SUCCEEDED(res))
    {
        ...
        u32 trampolineSize = cthulhuPostBootTrampolineEnd -
                             cthulhuPostBootTrampolineStart;
        u32 resumeOffset = cthulhuPostBootResumeLiteral -
                           cthulhuPostBootTrampolineStart;
        if (!header || trampolineSize > sizeof(header->reserved) ||
            resumeOffset + sizeof(u32) > trampolineSize)
        {
            res = MAKERESULT(RL_PERMANENT, RS_INVALIDSTATE, RM_LDR,
                             RD_TOO_LARGE);
        }
        else
        {
            memcpy(header->reserved, cthulhuPostBootTrampolineStart,
                   trampolineSize);
            *(u32 *)((u8 *)header->reserved + resumeOffset) =
                context.cpu_registers.pc;
            u32 trampolineVa = 0x07000000 +
                ((u8 *)header->reserved - (u8 *)header);
            svcFlushEntireDataCache();
            svcFlushProcessDataCache(process, trampolineVa, trampolineSize);
            context.cpu_registers.pc = trampolineVa;
        }
        context.cpu_registers.cpsr &= ~0x20; // Bootstrap is ARM code.
        if (R_SUCCEEDED(res))
    ...
    if (R_SUCCEEDED(res))
    {
        *threadIdOut = selectedThread;
        ...
        {
            value = *handshake;
            if (value == CTHULHU_BOOT_READY || value == CTHULHU_BOOT_FAILED)
                break;
            svcSleepThread(10 * 1000 * 1000LL);
        }
        if (value == CTHULHU_BOOT_READY)
            g_lastStage = "complete-framework-confirmed";
