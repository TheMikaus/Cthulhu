...
## 2026-08-18 — RC30 planned correctly but live updater ignored coordinates; RC31 direct placement

User result: RC30 did not visibly work. Logs prove request generation was not the failure. RC30 planned 187 title mutations (181 top-level plus six folder members), included all 14 positioned Launcher-backed system/DSi titles, assigned the folder coordinate 0, and assigned Camera from old 222 to new 92. Targeted Launcher commit wrote and verified one folder plus 14 titles with result 0; the SD shutdown merge also completed with result 0.

The live-map exposed the actual defect: `inline_changed=0` and `indirect_changed=0`. Both updater passes first collected only coordinates already occupied by recognized title records, then permuted the desired records across that same occupied-coordinate list. This preserves all holes by construction. If the recognized records were already alphabetically sequenced across their sparse positions, it reported zero changes even though the planned coordinates were different. Thus Camera could remain at live coordinate 222 while its backing Launcher position was already 92. This also explains the long-standing symptom that Collapse persisted after reconstruction but did not close gaps live.

RC31 records the desired top-level record for every exact `g_sortGrid` coordinate while resolving identities. After the compatibility permutation, both inline and indirect maps receive a coordinate-aware pass across the 360 top-level positions: a desired title record is written to its actual planned coordinate; an old coordinate is cleared only if it currently contains a recognized top-level title and no title is desired there. Unknown/non-title/package records are never cleared by this pass. The indirect pass is no longer incorrectly gated on whether the inline pass changed first. Any indirect coordinate change now requests the already-validated native refresh.

Visible release `0.1.0-rc31`, runtime `1.10.7`, scan `2.5.1`. Build succeeded. Deployment: `H:\luma\payloads\LumaHome010RC31.firm`; SHA-256 `6E9CCDF54874726AD224F677F74619F2BBC9C74791A0894AC596EC775EA81CD3`. RC30 archived as `LumaHome010RC30-NONCOORDINATE-LIVE-SORT.firm`; root firmware remains unchanged.

Focused RC31 test: boot RC31, verify the version, and Apply the remembered A-Z / folder Before / Collapse ON / column-major choices exactly once. Check immediately whether gaps close, the folder occupies the first coordinate, and Camera moves into the N section. Confirm input and HOME remain responsive. Then normal power-off/reboot and verify persistence before reinserting. A crash is possible because this is the first exact-coordinate live map write across the complete discovered title set; if it occurs, stop and preserve the dump and logs.

## 2026-08-18 — RC31 was not applied; RC30 reconstruction rewrapped titles

The user booted RC31 but did not run a new sort. On that boot, the already-committed RC30 layout reconstructed into an apparently sorted/compacted layout, but many SD titles appeared as gift packages again. Therefore this result does not test RC31's new exact-coordinate live writer. Do not ask the user to Apply RC31 until the newly available package-state evidence is captured.

Screenshots `HNI_0054.JPG` and `HNI_0055.JPG` were inspected. The first showed numerous orange gift packages interspersed through otherwise compacted titles; the second showed a mostly empty tail page with one gift package remaining. Both screenshots and their 32-byte camera sidecars were removed after inspection. The RC30 transaction itself had a collision-free plan: 181 top-level title positions were unique and exactly covered 1 through 181, while the folder occupied 0. Camera was planned from 222 to 92. Thus simple duplicate target coordinates are not the cause.

This revives package/presentation ownership as an active blocker despite the earlier decision to defer it: the system generated wrapped objects during reconstruction even though the user had unwrapped everything before. The current gift state is now the best available diagnostic condition. Next action should be RC31 X capture only, with no Apply, while these gifts are visible. Compare `object-inventory-rc31.csv` against RC29/RC30 to locate package records, associated title records, word/flag changes, and inline/indirect ownership. Do not mutate or unwrap until that capture is saved.

Payload cleanup requested by the user: all obsolete `LumaHome*.firm` payloads were removed from `H:\luma\payloads`. Only `GodMode9.firm` and `LumaHome010RC31.firm` remain, so the chainloader list fits on screen. The removed builds remain recoverable from Git/build history; they are no longer on the SD card. Root `H:\boot.firm` remains unchanged at SHA-256 `10A8356230FF4C3E7D72FCFBC2F7E47CC12717DE2B6AF5122E081B51E023CC2A`.

## 2026-08-18 — RC31 gift-state capture identifies a backing-store invariant

The user performed RC31 X capture without Apply. It succeeded read-only: SD discovery result 0, raw `346ca1e0`, processed `346ccf90`, resident Launcher `346bf208` with five matches, final result 0. `object-inventory-rc31.csv` and `live-map-0.1.0-rc31.txt` are valid.

Against the pre-regression RC29 capture, twelve records changed from normal title objects (typical word4 values `0080ffff` through `0099ffff`, word14 `3`) to gift/package objects with word4 `0000ffff` and word14 `1`. They occupy the reconstructed tail around combined coordinates 171–182. The affected identities include TWiLight/booter, Universal-Updater, Weapon Shop, Xeodrifter, YouTube, several Zelda titles, and 3D Altered Beast; the exact title shown at a coordinate can be grid-derived, so record flags and both layers are stronger package evidence than the coordinate label alone.

RC30's combined plan had no duplicate targets: the folder used 0 and 181 top-level objects uniquely covered 1–181. The failure is therefore not a collision. The evidence instead shows that SD SaveData and Launcher positions are separate backing-store orderings with HOME-specific merge/reconstruction rules. RC30 wrote one globally interleaved combined order into both stores, leaving holes throughout the SD title position sequence wherever system icons were inserted. HOME reconstructed the SD titles beyond the valid dense sequence as gift objects. A manually movable system icon does prove the UI coordinate is movable, but it does not prove the two serialized position arrays can be rewritten as one shared namespace.

Design correction: split each mutation into (a) a combined live HOME coordinate used for the inline/indirect maps and (b) a backing-store persistence coordinate. SD persistence must maintain HOME's valid dense SD ordering; Launcher persistence must maintain its own valid ordering. The live map can then express the merged alphabetical order without copying that merged coordinate verbatim into both files. Until this split is implemented and a repair path restores a valid SD sequence, RC31 Apply is unsafe and must not be tested. RC31 X capture remains safe.

## 2026-08-19 — RC32 separates live layout from dense persistence

RC32 implements the split required by the RC31 gift capture. The normal planner still builds the requested merged alphabetical layout, including system icons and folders, and saves each mutation's live coordinate plus a copy of the combined SD raw object before any persistence rewrite. `g_sortGrid`, `g_liveRaw`, live Launcher writes, and the inline/indirect exact-coordinate updater therefore receive the requested combined layout.

Before writing either archive, RC32 independently serializes a media-grouped traversal. Folder placement occupies its requested edge group, all positioned Launcher-backed titles form one contiguous traversal group, and all top-level SD titles form the next contiguous traversal group. Ordering inside each group still follows the selected A-Z/Z-A request. Folder members retain their independently sorted internal positions. The code validates that every persisted top-level title/folder coordinate is in range and globally unique; errors `-124`, `-125`, or `-126` occur before backup/commit if count, title, or folder invariants fail. Transaction details now log `live` and `persist` separately and include a `[PERSISTENCE_SPLIT]` section with Launcher, SD, folder, and combined counts.

This layout is intentionally not guaranteed to reconstruct the exact interleaved live alphabetical order after reboot; that requires learning HOME's native cross-store merge metadata. Its immediate safety purpose is to eliminate holes in the SD persistence traversal and repair the RC30 layout that generated gift objects. RC32 keeps RC31's direct coordinate-aware live updater. Legacy title-only callers initialize their own live raw/title snapshots so the new split cannot reuse stale static data.

Visible release `0.1.0-rc32`, runtime `1.10.8`, scan `2.6.0`. Build succeeded. Deployment: `H:\luma\payloads\LumaHome010RC32.firm`; SHA-256 `640F886A51AF82FB5544103973A93F346E3377AA0AC22EE0D4B158B84FF40B76`. RC31 was removed from the SD rather than archived to preserve the user's short chainloader list. Only GodMode9 and RC32 remain. Root firmware remains unchanged.

Focused RC32 repair test: do not manually unwrap or move anything first. Boot RC32, verify the visible version, choose the remembered A-Z / folder Before / Collapse ON / column-major settings, and Apply exactly once. Record whether the live layout changes or crashes, but do not judge package repair until a normal power-off and reboot. After reboot, check whether the twelve newly generated gifts reconstruct as normal icons, whether the layout remains usable, and whether L+Y still works. Then reinsert immediately. Inspect `[PERSISTENCE_SPLIT]`, live map counts, targeted Launcher commit, shutdown merge, and the postboot inventory/audit before another Apply.

## 2026-08-21 — RC32 fully succeeded; RC33 uses media-local persistence

User confirmed the RC32 test worked. The logs support the result: transaction result 0 with 187 title mutations and one folder; `[PERSISTENCE_SPLIT]` validated 14 top-level Launcher titles, 167 top-level SD titles, one folder, and 182 combined objects. The live exact-coordinate updater performed 187 inline changes and 363 indirect changes with no missing desired identity. Targeted Launcher commit verified one folder plus 14 titles; shutdown merge matched all 173 SD identities and wrote positions only, result 0. The user confirmed the generated gifts returned to normal and the resulting layout worked. RC32 is the first successful baseline that combines generic system-title discovery, exact live placement, and package-safe reconstruction.

The postboot persistence audit showed all 167 top-level SD positions differed from RC32's expected snapshot while HOME remained correct and the live resident raw/processed buffers matched HOME exactly. This is strong evidence that HOME normalized RC32's SD group from the globally offset range 15–181 into the SD store's own local dense range 0–166. Therefore the separate stores do not merely require contiguous groups; their position namespaces are media-local and may overlap numerically.

RC33 keeps RC32's proven combined live grid unchanged but persists SD and Launcher independently. Top-level SD titles receive a dense traversal over 0 through `sd_top-1`. Launcher titles and folders share their own dense traversal over `launcher_objects`, with folder Before/After applied inside that Launcher namespace. Validation now uses separate occupancy maps for SD and Launcher, allowing the expected numeric overlap while still rejecting duplicates inside either store. Folder members remain independently dense. This should eliminate HOME's 167-position postboot normalization and produce a stable expected/current persistence audit without reintroducing gifts.
...
## 2026-08-21 — RC34 exposes hybrid namespace; RC35 persists Launcher globally

RC34 test sequence: user selected folder After and column traversal. Before reboot the first icon appeared wrapped. Starting an application caused the folder to return from the live After position to its original front position. The user sorted again; after reboot, some front icons appeared at the end.

The RC34 per-title audit resolves the apparent SD persistence mismatch. For all uniquely matched SD identities, `identity_position_mismatches=0` and folder membership was unchanged. The old slot-index comparison reported 167 position mismatches because HOME permuted 167 internal SaveData slots—mostly a one-slot rotation with 3D Altered Beast moving from expected slot 166 to current slot 0—while preserving each title's exact intended position. Slot index is not identity and must not be used to judge position persistence. RC35 adds summary counters for slot permutations, identity-position mismatches, and missing/ambiguous identities.

The folder rollback proves Launcher persistence timing and namespace were wrong in RC34. Live folder After was combined coordinate 181, but RC34 planned local Launcher persistence coordinate 14 and did not write it until shutdown. An application transition reloaded the still-old on-disk coordinate 0, immediately undoing the live move. More broadly, manual Camera movement and this folder behavior show Launcher positions are combined/global coordinates, while only SD SaveData positions require media-local dense serialization.

RC35 uses the hybrid model: SD top-level titles persist in their safe local dense traversal; every Launcher-backed title and folder persists its actual combined live coordinate. Validation still uses separate SD and Launcher occupancy maps. Immediately after HOME is unlocked, RC35 attempts the same exact-ID/number targeted Launcher commit that was previously delayed until shutdown. The plan remains armed and shutdown re-verifies it. Journal stage `apply-launcher-committed` versus `apply-launcher-deferred` identifies whether the archive accepted the immediate write. This should prevent application transitions from restoring the old folder location and should preserve the combined system/folder placement across reboot.

The first-icon wrapping observed live remains a separate open issue. RC34's pre-sort inventory showed 3D Altered Beast as a normal record (`word4=008affff`, `word14=3`) at coordinate 182; the live direct pass intended it at coordinate 0. RC35 does not claim to fix that transient presentation symptom. If it recurs, capture X immediately while the wrapped first icon is still visible, before launching an application or sorting again.

Visible release `0.1.0-rc35`, runtime `1.11.1`, scan `2.7.0`. Deployment: `G:\luma\payloads\LumaHome010RC35.firm`; SHA-256 `897B9F0E59290D59D5C498C192CA8CA69C4293C0EF880ACEB9F82149739DE8B3`. RC34 was removed from the SD; only GodMode9 and RC35 remain. Root firmware unchanged.

Focused RC35 test: boot RC35; Apply A-Z, folder After, Collapse ON, column traversal once. If the first icon wraps, press X immediately to capture before doing anything else. Then start and exit one application and verify the folder remains After. Normal power-off/reboot and verify system/front icons and folder preserve the live combined placement. Reinsert. Inspect immediate Launcher commit result, shutdown verification, identity-based audit counters, and any package-state capture.

## 2026-08-21 — RC35 immediate Launcher commit is unsafe; RC36 restores shutdown ownership

RC35 failed its application-transition test. Applying folder After wrapped the first icon. After launching an application, attempting to return HOME left the system in an abnormal state: HOME Menu did not open and the user could not return to it. No ARM11 crash dump was produced. The card logs confirm RC35 executed its new immediate targeted Launcher.dat write while HOME was active: `[LIVE_FOLDER_WRITE]` wrote 15 records successfully, and `launcher-commit.txt` reports one folder plus 14 Launcher titles written and verified. A later shutdown journal also completed with result 0. This makes the immediate successful archive mutation—not a failed write or exception—the leading cause of the broken application-return reload path. HOME must not have Launcher.dat changed underneath its active process.

RC36 removes only the immediate `WriteLauncherFolderPositions()` call after the live update/HOME unlock. The original live-sort hook, exact-coordinate inline/indirect mutation, resume repair, hybrid persistence coordinates, and shutdown observer remain. Apply records `apply-launcher-deferred`; the shutdown observer exclusively owns the durable targeted Launcher commit. This deliberately restores the safe persistence timing used through RC34 while retaining RC35's hybrid hypothesis (SD positions media-local; Launcher titles and folders at combined live coordinates) for a controlled reboot-only test. Do not launch an application during the first RC36 test.

The first-icon gift is still unresolved. RC35's live map has `inline_changed=149`, `indirect_changed=167`, no missing desired records, and folder coordinate 181 already present in both layers. No `capture-report-rc35.txt` exists, so the RC35 inventory cannot be treated as a post-wrap X capture. Coordinate zero or the interaction between HOME's inline and indirect ownership remains suspect, but RC36 intentionally does not combine that investigation with the safety rollback.

Visible release `0.1.0-rc36`, runtime `1.11.2`, scan `2.7.1`. Build succeeded. Deployment: `G:\luma\payloads\LumaHome010RC36.firm`; SHA-256 `3159BBB11AA3D271228AD6F098CE738EB86CC3CCC1F64C0ACE62B1E76C388935`. RC35 was removed from the SD; only GodMode9 and RC36 remain. Root `G:\boot.firm` remains SHA-256 `10A8356230FF4C3E7D72FCFBC2F7E47CC12717DE2B6AF5122E081B51E023CC2A`.

Focused RC36 test: boot RC36 and verify the visible version. Apply the same A-Z, folder After, Collapse ON, column traversal request exactly once. Do not launch any application. If the first icon wraps, press X immediately to capture while it is visible. Then perform a normal power-off directly, reboot RC36, and inspect whether the folder and Launcher/system icons reconstruct in the requested combined positions without gifts. Reinsert the card immediately afterward. This isolates shutdown-only hybrid persistence from the unsafe active-HOME commit.

## 2026-08-21 — Memory-boundary documentation audit

3dbrew documents the ARM11 process virtual-address architecture, but not fixed addresses or allocation sizes for HOME Menu's private icon/model objects. Ordinary private heap mappings are placed within `0x08000000..0x10000000`; shared memory occupies `0x10000000..0x14000000`; legacy linear heap is `0x14000000..0x1C000000`; VRAM is `0x1F000000..0x1F600000`; configuration/shared pages begin at `0x1FF80000`; newer BASE/system linear memory is `0x30000000..0x38000000` on Old3DS and may extend through `0x40000000` on New3DS. Physical FCRAM is 128 MiB on Old3DS and 256 MiB on New3DS. These are architectural windows, not proof that an arbitrary pointer or a 360-entry object array is valid throughout a window.

The documented `QueryMemory`/`svcQueryProcessMemory` interface returns the actual mapping base, size, permissions, and state containing a queried address. Therefore all HOME private-object dereferences and array spans must be validated as `pointer >= mem.base_addr` and `pointer + required_bytes <= mem.base_addr + mem.size`, with overflow checks and required read/write permission, immediately before use. The current code correctly uses `svcQueryProcessMemory` in discovery paths but still has multiple broad pointer gates of the form `>= 0x08000000 && < 0x40000000`; those are classification checks only and are insufficient as safety boundaries. A future hardening iteration should centralize range validation and apply it to the raw model, processed model, inline record array, indirect map, owners, and inferred child pointers. The runtime region dump from this console previously showed shared/static windows at `0x10000000/0x1000`, `0x10002000/0x1000`, `0x18000000/0x332000`, and `0x1FF80000/0x2000`; these corroborate that mapped regions are discrete rather than one continuous valid span.

## 2026-08-21 — RC37 validates complete live-model spans

RC37 implements a centralized `ValidateRemoteSpan` helper around `svcQueryProcessMemory`. It rejects zero-size or wrapping spans, free/empty mappings, spans extending beyond the queried mapping, and mappings missing the required permissions. The live icon mutation path no longer treats `0x08000000..0x40000000` as sufficient pointer validation.

Before dereference or mapping, RC37 validates the inferred model-record pointer, the diagnostic 12-record prefix, the complete 420-by-`0x230` record array, the full inline header plus 420 signed-16-bit entries, and the complete 420-entry indirect map. Page-aligned fixed mapping windows are separately checked so a valid object near a mapping boundary cannot authorize an oversized mapping. Pointer-offset additions for the model record pointer and index header are guarded against 32-bit overflow. Inventory-only record and indirect mappings receive the same read-span checks; mutation maps require read/write permission.

The RC37 live-map adds the actual queried base, size, permissions, and state for the model pointer, full record array, inline header, and indirect map. Validation failures use local results `-130` (invalid/overflowing span), `-131` (outside mapping/free mapping), or `-132` (missing permission) and are logged instead of permitting a mutation. This iteration deliberately does not change sorting, coordinate-zero behavior, folder planning, live refresh, or shutdown-only persistence; it is a safety and evidence boundary around the existing RC36 behavior.

Visible release `0.1.0-rc37`, runtime `1.11.3`, scan `2.8.0`. Build succeeded. Deployment: `G:\luma\payloads\LumaHome010RC37.firm`; SHA-256 `611D379DE3F1473FFF4D87B7919C522FE4CE960B8863F2933B9AD17377FDEDBE`. RC36 was removed from the payload directory; only GodMode9 and RC37 remain. Root `G:\boot.firm` remains SHA-256 `10A8356230FF4C3E7D72FCFBC2F7E47CC12717DE2B6AF5122E081B51E023CC2A`.

Focused RC37 test: boot RC37 and verify the visible version. Open L+Y and Apply the remembered sort once. If Apply refuses or reports failure, do not retry; power off and reinsert so the new region/result fields can identify the rejected span. If it applies, note whether the first icon wraps, but do not launch an application. Press X immediately if wrapping occurs, then perform a normal power-off/reboot and reinsert. This both verifies that the known HOME objects fit documented runtime mappings and preserves the RC36 isolation of shutdown-only persistence.

## 2026-08-21 — RC37 stopped before live mapping; RC38 recovers active folder metadata

User result: the folder did not move properly. RC37 logs show that no live mutation occurred. Runtime result and transaction result were `0xFFFFFFE7` (`-25`), `folder_mutations=0`, every title's provisional live position remained zero, and no RC37 live-map or capture was produced. Thus the RC37 memory-span validator neither rejected nor modified a live object; planning stopped first.

The `-25` guard rejected duplicate old positions after combining SD and Launcher titles. Current SD persistence positions are media-local while Launcher positions are combined/global, so equal numeric values across those stores are legitimate. With Collapse ON those old numbers do not determine targets at all: the sorted request index feeds `CompactTraversalPosition`. RC38 retains the duplicate-position guard only for non-collapse mode, where the original sparse positions still matter.

The second failure was folder discovery. The current SD layout still has six titles referencing folder ID 0, and the existing `pre-sort-Launcher.dat` contains folder 0 identifier 1 at position 181. However Launcher.dat was locked (`0xC92044E7`) and every discovered resident Launcher candidate reported zero credible folder metadata. RC38 adds a narrow fallback: only when the archive read is invalid and resident folder metadata count is zero, read and validate `pre-sort-Launcher.dat`; for each folder ID actively referenced by a current SD title, copy only that folder's position, identifier, 17-character name, and row count into the planning image. It never imports backup title IDs, title positions, or membership, and ignores unreferenced/deleted backup folders. `[FOLDER_METADATA_FALLBACK]` logs resident count, read/validation result, and recovered count.
...
## 2026-08-21 — RC39 folder persisted; RC40 makes row-major wrapping page-aware

After reboot, the folder was in the correct place, but the user reported icons in incorrect positions and supplied `HNI_0060.JPG`. The image showed a sparse-looking 5-row layout. It and its 32-byte sidecar were inspected and removed from the SD afterward. RC39 logs confirm a successful Z-A, folder Before, Collapse ON, row-major transaction; the current display setting was correctly read as five rows, the folder persisted at coordinate 0, shutdown commits verified, and all new memory-span checks passed inside HOME's `0x34332000+0x01320000` RW mapping.

The defect was not failure to read the selected icon size. `CompactTraversalPosition` derived the row-major column count from all 182 occupied objects, treating the complete 360-coordinate HOME strip as one enormous 37-column row set. The 3DS HOME layout is divided into 60-cell pages; the selected row count determines columns per page (five rows means a 5x12 page). Therefore row-major order must wrap within each 60-cell page, then continue on the next page.

RC40 defines `CTH_HOME_PAGE_SLOTS=60`. Row-major compact placement now maps rank to page, row, and column using `columns=60/rows`, and sparse/non-collapse traversal keys use the same page-aware transformation. Column-major behavior is unchanged. This still sorts the current exact displayed title records by identity; it does not intentionally replace a title's normal/package record. The visible panel now reads `LUMAHOME 0.1 RC40`; release `0.1.0-rc40`, runtime `1.11.6`, scan `2.9.0`.

Build succeeded after fixing an unused-parameter warning. Deployment: `G:\luma\payloads\LumaHome010RC40.firm`; SHA-256 `537A7AF9709702861FFCDD92AA4068420B4CB61AD81334E1481675B5C786D134`. RC39 was removed; only GodMode9 and RC40 remain. Root firmware unchanged. Focused test: boot RC40, verify the visible header, apply the same Z-A / folder Before / Collapse ON / row-major request once, and inspect the live ordering across the first page boundary (after 60 cells). Power off/reboot and verify the same page-aware wrap persists; take one screenshot if incorrect, then reinsert.

## 2026-08-21 — RC40 used logical capacity, not visible width; RC41 viewport wrap and fast discovery

User reported RC40 icons still looked wrong and supplied `HNI_0061.JPG`. The screenshot and sidecar were inspected and removed. RC40 logs show a successful Z-A, folder Before, Collapse ON, row-major transaction with `home_rows=5`, one folder, 187 title mutations, successful shutdown persistence, and valid memory spans. However the live-map was partial: the inline map still had one stale folder-member record while the indirect map had the folder object at coordinate 0. More importantly for the pictured ordering, RC40 wrapped at the 60-cell logical block rather than the actual visible viewport width.

RC41 uses HOME's visible zoom geometry: visible columns equal selected rows plus two (3x5, 4x6, 5x7). Row-major traversal now wraps after `rows * (rows + 2)` coordinates, then continues in the next horizontal viewport. This preserves dense physical coordinates while making left-to-right/top-to-bottom order correspond to what is visible on screen. Column-major behavior is unchanged.

Apply latency was also traced. `ScanLiveIconClassV010Rc8` mapped and searched about 24 MiB of HOME memory twice; RC40 logged 5,978,112 32-bit words. Sorting 187 entries is insignificant compared with this discovery. RC41 first validates the known USA HOME owner at `0x08032918`: the full owner span must be readable, its `+0x120` function field must equal `0x001CA504`, its `+0x128` model pointer must exist, and the entire model mutation span through the 420-entry inline map must be RW inside one current mapping. Only then is the fast path used. If any signature/span check fails—such as after a changed HOME build or relaunch—it automatically performs the existing full scan. The live map logs `fast_path` and `fast_result`.

Visible release `LUMAHOME 0.1 RC41`, runtime `1.11.7`, scan `2.10.0`. Build succeeded. Deployment: `G:\luma\payloads\LumaHome010RC41.firm`; SHA-256 `17D9B21828ECA84DC12698AD01954EB699A4B11D3E5E9D3940364756515C2537`. RC40 removed; only GodMode9 and RC41 remain. Root firmware unchanged. Test: boot RC41; apply the same five-row Z-A/Before/Collapse/row-major request and compare responsiveness plus ordering within each seven-column viewport. Do not launch an application. Power off/reboot, verify persistence, and reinsert. The key logs are `fast_path=1`, near-zero scan `words`, no sort error, and the screenshot result if ordering is still incorrect.

## 2026-08-21 — RC41 fast discovery passed; RC42 compacts the final partial viewport

The user reported that icons did not remain in the exact expected positions and supplied `HNI_0062.JPG` and `HNI_0063.JPG`. Both screenshots and sidecars were inspected and removed. RC41's discovery optimization passed: the live-map reported `fast_path=1` and `words=0`, so Apply used the validated known owner/model and no longer performed the multi-megabyte scan. The transaction itself completed as Z-A, folders After, Collapse ON, row-major, five rows, with 187 title mutations and one folder mutation.

The screenshots exposed a deterministic layout error. RC41 always used all seven visible columns for every viewport. With 182 combined top-level objects, the final viewport contains only seven objects, but the last-ranked After folder was mapped as if that viewport contained 35 objects: coordinate 205. That left coordinates 181–204 unused and visually isolated the folder at the far side despite Collapse being enabled. The transaction log confirms `folder live=205 persist=205`; this was not a stale layout-setting read.

RC42 retains the selected-row detection and full seven-column geometry for complete 35-object viewports, but calculates the number of needed columns for the final partial viewport as `ceil(remaining / rows)`. For the present five-row/seven-object tail, it uses two columns, maps the seven objects in row-major order inside that compact block, and places the last-ranked folder at coordinate 178 rather than 205. This preserves left-to-right/top-to-bottom rank while eliminating the large artificial tail gap. Column-major behavior is unchanged. RC41's validated fast discovery and all memory, hook, persistence, and folder safeguards remain intact.

Visible release `LUMAHOME 0.1 RC42`, runtime `1.11.8`, scan `2.10.1`. Build succeeded. Nested commit `1d0d886` was pushed to `lumahome/home-menu-framework`. Deployment: `G:\luma\payloads\LumaHome010RC42.firm`; SHA-256 `452CD71986AF8FBE69C03B97EDCC6FE3C7C9DBFB3CCCD17A774260CD8AEE449B`. RC41 was removed; only GodMode9 and RC42 remain. Root firmware SHA-256 remains `10A8356230FF4C3E7D72FCFBC2F7E47CC12717DE2B6AF5122E081B51E023CC2A`.

Focused RC42 test: boot and verify the panel reads RC42. Keep the same five-row, Z-A, folders After, Collapse ON, row-major settings and Apply once. The final seven-object group should form a compact two-column block; the folder should be the last object in visual row-major reading order rather than isolated at the far right. Then power off normally, reboot once, and verify the same positions persist. If wrong, take one screenshot and reinsert without applying a second sort so the RC42 transaction and live-map describe the exact pictured state.

## 2026-08-21 — RC43 defines row-major order by fixed visible screens

Before testing RC42, the user clarified the intended traversal contract: the selected visible screen is the bound. For a seven-column/five-row display, fill all 35 positions on the current screen left-to-right and top-to-bottom, then continue at the first position of the next screen. A partial final screen retains the selected seven-column width; it must not shrink according to the number of remaining objects.

RC43 removes RC42's variable-width final-screen compaction and makes the rule explicit in `CompactTraversalPosition`. Rank is divided by `rows * (rows + 2)` to select the screen, and the within-screen rank selects row then column. Thus ranks 0–34 fill screen one, 35–69 fill screen two, and so forth. For five rows, ranks in each screen map through seven fixed columns. This is the user's requested screen-bounded order even though the column-major HOME coordinate values can contain numeric holes under a partially filled final row. Column-major sorting is unchanged. RC41's fast validated model discovery remains intact.

Visible release `LUMAHOME 0.1 RC43`, runtime `1.11.9`, scan `2.10.2`. Build succeeded. Nested commit `66a9c6a` was pushed to `lumahome/home-menu-framework`. Deployment: `G:\luma\payloads\LumaHome010RC43.firm`; SHA-256 `3A983D48CDB627F333F03E2606E9C0F81BAF85EA6E48E1E17DBA4CB2EB8C3E2B`. RC42 was removed; only GodMode9 and RC43 remain. Root firmware hash remains unchanged.

Focused RC43 test: boot and verify RC43. With five rows, row-major, Collapse ON, and the desired folder placement, Apply once. Check that each visible seven-by-five screen is filled across rows before traversal continues to the next screen. In particular, inspect the boundary between object 35 and 36 and the final partial screen. Then power off/reboot and verify the same screen-bounded order persists.

## 2026-08-21 — RC44 uses the verified geometry for all six icon-size selections

The user clarified that seven columns/five rows was only an example and required the sorter to understand every HOME Menu icon-size selection. RC43 was therefore superseded before user testing. Research confirmed that Launcher.dat stores rows-minus-one in range 0–5 (one through six rows), while the visible grids are 1x3, 2x3, 3x5, 4x6, 5x8, and 6x10. The former `rows + 2` shortcut was only correct for the three- and four-row selections and was wrong for one, two, five, and six rows.

RC44 adds a single explicit `VisibleHomeColumns` table `{1:3, 2:3, 3:5, 4:6, 5:8, 6:10}`. Both `TraversalKey` (used to interpret existing sparse positions) and `CompactTraversalPosition` (used by Collapse) call this same function. Row-major sorting fills the selected fixed-size screen left-to-right/top-to-bottom, then advances to the next screen; a partial final screen retains that selection's normal width. Invalid row counts fall back safely instead of dividing by an invented geometry. Column-major behavior remains unchanged.

Sources consulted: 3dbrew HOME Menu documents the row field, 1–6 range, linear positions, and 60-entry chunks; a published Nintendo 3DS HOME Menu layout reference enumerates the six visible row/column combinations. Visible release `LUMAHOME 0.1 RC44`, runtime `1.12.0`, scan `2.11.0`. Build succeeded. Nested commit `6094a3c` was pushed to `lumahome/home-menu-framework`. Deployment: `G:\luma\payloads\LumaHome010RC44.firm`; SHA-256 `FE1D7B811A6FD68D7A3AD7E46FEB4F4963728F131A1F335ABAD754517A7AE2A6`. RC43 was removed; only GodMode9 and RC44 remain. Root firmware hash remains unchanged.

Focused RC44 test: boot and verify RC44. Select the icon size desired for normal use, then Apply a row-major Collapse sort once. Verify that icons fill every row across the entire visible screen before continuing on the next screen. Power off/reboot and verify the same ordering persists. A later regression pass should repeat this at each of the six zoom selections, especially two rows (three columns), five rows (eight columns), and six rows (ten columns), since those distinguish the verified table from the old formula.

## 2026-08-22 — RC44 failed closed on a false Launcher candidate; RC45 fixes resident-offset validation

User test: RC44 reported that sorting failed. The card was already mounted as G:. Runtime and transaction logs show result `0xFFFFFF8F` (`-113`), zero title/folder mutations, and `home_rows=179`. Valid HOME row counts are 1–6, so the transaction stopped before modifying live memory or scheduling persistence. The geometry table itself was not reached.

Discovery reported four Launcher-like candidates and selected `0x346CCF90`, despite the known preferred resident address being `0x3469F208`. The scanner's structural check was misaligned: HOME's resident Launcher object begins at Launcher.dat file offset +8, but `DiscoverLauncherRuntime` passed that resident pointer directly to `ValidateLayout`, which expects the file header at offset zero. Generic title/layout-shaped buffers could therefore pass the wrong interpretation. The selected false match produced row byte 178 (reported as 179 after adding one).

RC45 adds `ValidateResidentLauncher`. For every quick-match candidate it reconstructs a temporary Launcher.dat planning image with format header 4 and copies the resident bytes at offset +8, exactly as the later transaction path does. It explicitly requires the reconstructed stored row byte to be 0–5 before applying the full layout validator. Both candidate selection and later planning now agree on the resident representation. RC44's complete screen-geometry table and row-major screen-bounded behavior are unchanged.

Visible release `LUMAHOME 0.1 RC45`, runtime `1.12.1`, scan `2.11.1`. Build succeeded. Nested commit `0308116` was pushed to `lumahome/home-menu-framework`. Deployment: `G:\luma\payloads\LumaHome010RC45.firm`; SHA-256 `255A4B113D9CCF378D7AEC72DA9455B6BFEAA534C813DCF3C76BA886EDD130DA`. RC44 was removed; only GodMode9 and RC45 remain. Root firmware hash remains unchanged.

Focused RC45 test: boot and verify RC45, then repeat the exact RC44 row-major Collapse sort once. Success requires no failure message and a transaction row count from 1 through 6. If it fails, reinsert immediately without retrying. If it succeeds, inspect the fixed-screen traversal, then power off/reboot once and confirm persistence.

## 2026-08-22 — RC45 sorted but wrapped at the wrong visible width; RC46 matches the target HOME build

User result: the first RC45 sort completed, but the visible order was wrong; after changing to the lowest icon-count selection, another sort appeared not to work. The supplied `HNI_0064.JPG` and sidecar were inspected and removed. RC45's transaction succeeded with 187 title mutations, one folder mutation, A-Z, folders Before, Collapse ON, row-major, and `home_rows=5`. The resident Launcher candidate fix passed: selected address `0x346BF208`, three validated candidates, and no invalid-row error.

The screenshot is decisive: the active five-row HOME layout displays ten complete columns, but RC45 used the older published 5x8 geometry. The alphabetical sequence therefore wrapped after eight ranks while two more columns were still visible on the same screen. This is a traversal-width error, not a title-name or live-map failure. The screenshot's first row contains the expected initial A-Z sequence through the eighth object, followed by objects from the next incorrectly started screen.

RC46 updates the target current-HOME geometry to columns `{1:3, 2:4, 3:6, 4:8, 5:10, 6:10}`. Both sparse traversal interpretation and compact placement still share the same function. Every transaction now logs `home_rows`, `home_columns`, and `screen_capacity`, so an attempted repeat at another zoom cannot leave the selected geometry ambiguous in the sorter report. Repeated Apply behavior in the overlay remains enabled; RC45's runtime snapshot and transaction contained only the successful five-row operation, so the reported lowest-count attempt did not leave evidence of a second sorter invocation.

Visible release `LUMAHOME 0.1 RC46`, runtime `1.12.2`, scan `2.12.0`. Build succeeded. Nested commit `98f37a8` was pushed to `lumahome/home-menu-framework`. Deployment: `G:\luma\payloads\LumaHome010RC46.firm`; SHA-256 `C8D9ED949E701B6E08C352FD2491F7651D5D3E3FDB71177F22CBDB06DE7C5BEF`. RC45 was removed; only GodMode9 and RC46 remain. Root firmware hash remains unchanged.

Focused RC46 test: boot and verify RC46. First use the same five-row selection and Apply A-Z/Before/Collapse/row-major once; the visible row must continue through all ten columns before wrapping. Then close the overlay, change to the largest-icon/lowest-count selection, reopen L+Y, and Apply once. If either result is wrong, stop after that Apply and reinsert. The transaction's explicit row/column/capacity values will distinguish wrong geometry detection from a sort that never ran.

## 2026-08-22 — RC46 proves the row source can disagree with the visible renderer

User result: RC46 still did not place icons correctly and supplied `HNI_0065.JPG`. The screenshot and sidecar were inspected and removed. The final RC46 transaction succeeded as Z-A, folders Before, Collapse ON, row-major, with 187 title mutations and one folder mutation. It logged `home_rows=4 home_columns=8 screen_capacity=32`. The screenshot visibly shows five rows and ten columns. Thus even RC46's corrected width for a given row count cannot work: the resident Launcher-derived row value used by the sorter did not describe the geometry rendered in the photographed state.

This is now treated as a geometry-source defect, not another table defect. Title comparison produced the expected reverse alphabetical sequence, but `CompactTraversalPosition` received four/eight geometry and wrapped ranks every 32 objects. The current placement code and its multiple coordinate namespaces were not documented clearly enough for an outside reader. Added `docs/SORT-PLACEMENT.md`, covering HOME's column-major native coordinate, `TraversalKey`, `CompactTraversalPosition`, authoritative combined title/folder placement, SD-local persistence regrouping, and live buffer commit. It explicitly records that the next correction must identify the active renderer zoom/row value (or prove which resident copy updates on enlarge/reduce) before changing geometry again.

No firmware behavior or installed payload changed in this documentation/diagnostic iteration. RC46 remains installed. Next engineering step: instrument the known render/model owners around the zoom transition and log the Launcher-derived row alongside candidate renderer/layout values, ideally sampling before and after enlarge/reduce in one boot. Do not make another placement-width guess first.

## 2026-08-22 — Documentation audit: persisted geometry is documented; active renderer geometry is not

At the user's request, local SDK/research material, 3dbrew HOME Menu documentation, and Nintendo HOME Menu material were checked specifically for a supported way to read active rows/columns. 3dbrew documents `Launcher.dat[0xB51]` as rows-minus-one (stored 0–5), `0xB5C` as cursor position, `0xB5E` as horizontal scroll (divide by rows for hidden columns), `0xD9A` as 360 linear `s16` icon positions, `0x1434+folderId` as each folder's row count, and the corresponding folder cursor/scroll arrays. Because HOME's resident object begins at file offset +8, this project's selected resident candidate exposes the saved main row byte at object offset `0xB49`.

No public Nintendo SDK/libctru/NS/APT API or documented HOME field was found for the immediately active renderer zoom, visible-column count, icon pitch, or viewport width. Nintendo confirms six enlarge/reduce stages and up to 60 simultaneously visible icons. An older public layout reference gives 1x3, 2x3, 3x5, 4x6, 5x8, 6x10, but that is UI geometry documentation, not a synchronized runtime API. Therefore the Launcher row field is authoritative for persisted/reboot layout, but RC46 proves the chosen resident copy is not necessarily authoritative for the currently rendered frame.

Expanded `docs/SORT-PLACEMENT.md` with this documentation boundary. No firmware or SD payload changed. The next runtime iteration must reverse-engineer or hook the renderer/layout state; documentation alone cannot supply the live value.

## 2026-08-22 — RC47 hooks HOME's active controller-row synchronization

The user authorized the next implementation. The existing local USA HOME code capture (`runtime/.analysis/HomeMenu-USA-code.bin`, 2,633,728 bytes) was disassembled with the pinned ARM toolchain. Direct references to documented `Launcher.dat[0xB51]` were traced. Runtime `0x002958D8` loads the saved rows-minus-one and stores it into the controller cache at outer-controller `+0x11C4`. Runtime `0x0021D428` reads the active cached value from inner-controller `+0x1C4`, then compares it to `Launcher.dat[0xB51]` and writes the active value back if they differ. This is the missing saved-versus-renderer boundary.

RC47 adds a signature-checked observer branch at `0x0021D428` (expected instruction `E59411C4`). The stub replays the displaced load, publishes active rows-minus-one to shared channel `+0x10C`, increments generation at `+0x110`, preserves scratch registers, and resumes at `0x0021D42C`. It does not alter the controller value or native synchronization. The complete injected stub is 0x644 bytes within the existing 0xE8C-byte validated code cave.

Before background Apply locks HOME, Rosalina snapshots the observed active value/generation. Row-major Apply now requires a generation and value 0–5, uses active+1 as effective rows, and fails closed with `-125` otherwise. Column-major retains persisted rows because it does not need a visual coordinate reinterpretation. `[TRAVERSAL]` logs source, persisted rows, active value, generation, effective rows, columns, and capacity. `runtime.txt` also continuously records active stored rows, decoded rows, and generation. The geometry table returns to the documented six states `{1:3, 2:3, 3:5, 4:6, 5:8, 6:10}`; RC47's observer will determine whether the photographed state was actually HOME's six-row controller state despite the stale resident Launcher value.

Visible release `LUMAHOME 0.1 RC47`, runtime `1.12.3`, scan `2.13.0`. Build succeeded. Nested commit `11a0563` was pushed to `lumahome/home-menu-framework`. Deployment: `G:\luma\payloads\LumaHome010RC47.firm`; SHA-256 `7618F89BC1B7826CDEF1C3FB813B12860E5B90C425E7D3288F2ED73BC4A2AD7B`. RC46 was removed; only GodMode9 and RC47 remain. Root firmware hash remains unchanged.

Focused RC47 test: boot and verify RC47. Before sorting, change the HOME icon size down one step and back to the desired setting so the native controller synchronization executes. Open L+Y and Apply one row-major Collapse sort. If Apply reports failure, stop and reinsert; `-125` means the observer did not execute and no mutation occurred. If it succeeds, inspect the row order once, take a screenshot only if wrong, then reinsert without a second sort. The runtime and traversal logs will directly compare persisted versus active rows.
